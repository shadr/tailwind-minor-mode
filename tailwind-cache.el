...
(defun tailwind-minor-mode--get-completions (root CALLBACK)
  (let ((tailwind-cache--connection (tailwind-minor-mode--cache-make-connection)))
    ...
    (jsonrpc-async-request tailwind-cache--connection
                           ...
                           :success-fn (lambda (result)
                                         ...
                                         (funcall CALLBACK (plist-get result :items))))))
...
(defun tailwind-minor-mode-cache-completions ()
  "Caches tailwindcss class completions for the current project."
  (interactive)
  (let ((root (project-root (project-current))))
    (tailwind-minor-mode--get-completions
     root
     (lambda (result)
       (setq tailwind-minor-mode--cached-completions
             (plist-put tailwind-minor-mode--cached-completions root
                        (tailwind-minor-mode--completions-to-hash-table result)))))))
